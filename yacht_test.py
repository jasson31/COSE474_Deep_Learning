...
def create_train_set(size_per_score_type):
    ...
    for score_type in range(score_func.index(yacht_score.SIXES), score_func.index(yacht_score.YACHT) + 1):
        if score_type != score_func.index(yacht_score.CHOICE):
            for i in range(size_per_score_type):
                already_set_scores = [randint(0, 1), randint(0, 1), randint(0, 1), randint(0, 1), randint(0, 1), randint(0, 1)]
                raw_dice_state = [0] * 5
                ...
                if score_type == score_func.index(yacht_score.FULL_HOUSE):
                    ...
                    for already_set_scores_index in range(len(already_set_scores)):
                        already_set_scores[already_set_scores_index] = 0
                ...
                roll_count_state = [randint(0, 2) / 2]
                ...
                bonus_state = [min(bonus_score, 35) / 35]
